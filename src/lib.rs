...
const WIDE_EASTASIAN : [(u32, u32); 106] = [
...
fn bisearch(ucs : u32, table : &[(u32, u32)]) -> i32 {
	let mut lbound = 0;
	let mut ubound = table.len() - 1;
...
pub fn wcswidth(s : &str) -> i32 {
    return s.chars()
            .fold(0, |sum, c| sum + wcwidth(c));
}
...
mod tests {
    ...
    #[test]
    fn test_hello_jp() {
        // ported from python lib's unit test
        ...
        let phrase = "コンニチハ, セカイ!";
        let expect_length_each = vec![2, 2, 2, 2, 2, 1, 1, 2, 2, 2, 1];
        let expect_length_phrase : i32 = expect_length_each.iter().sum();

        let length_each = phrase.chars().map(|c| wcwidth(c)).collect::<Vec<i32>>();
        let length_phrase = wcswidth(&phrase);
        assert_eq!(length_each, expect_length_each);
        assert_eq!(length_phrase, expect_length_phrase);
    }
}
